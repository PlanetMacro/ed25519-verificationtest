/- Spec for the transpiled schoolbook multiplication (radix-2⁵¹, 19-folded):
   no panic under the 2⁵⁴ invariant (the in-code debug_asserts become provable
   `massert`s), output limbs < 2⁵¹ + 2¹³, and the denotation multiplies in 𝔽_p.

   The proof script mirrors the generated body step by step (let* + named
   posts), interleaving explicit bound facts so every overflow side condition
   is linear; generated by /tmp/gen_mul_proof.py, then hand-tuned. -/
...
/-- Discharge tactic shared by most steps. -/
macro "dis" : tactic =>
...
theorem mul_spec (a b : Fe) (x0 x1 x2 x3 x4 y0 y1 y2 y3 y4 : U64)
  ...
  -- b*_19 precomputations + limb loads
  ...
  -- c0 = a0*b0 + 19*(a4*b1 + a3*b2 + a2*b3 + a1*b4)
  ...
  have hnc4 : c4.val = x4.val*y0.val + x3.val*y1.val + x2.val*y2.val + x1.val*y3.val + x0.val*y4.val := by
    simp only [c4_post, i48_post, i46_post, i44_post, i42_post, i43_post, i45_post, i47_post, i49_post,
              ...
              he_i13, he_i16]
    try ring
  -- 𝔽_p bridge: A·B = Σ cᵢ·2⁵¹ⁱ using 2²⁵⁵ = 19
